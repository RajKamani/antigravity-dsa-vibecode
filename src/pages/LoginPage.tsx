...
import { LogIn, Infinity as InfinityIcon } from 'lucide-react';
...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(err.response?.data?.detail || 'An error occurred during login', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--c-bg)] py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-[var(--c-accent)] opacity-[0.15] blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-[var(--c-success)] opacity-[0.1] blur-[120px] pointer-events-none" />
      
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-10">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[var(--c-accent)] to-[#8b5cf6] flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 mx-auto mb-6 transform transition-transform hover:scale-105">
          ...
          <p className="mt-2 text-sm text-[var(--c-text-3)] font-medium">
            Sign in to continue your O(1) Knot journey
          </p>
        </div>
