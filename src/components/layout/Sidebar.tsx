...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-60 bg-[var(--c-panel)] border-r border-[var(--c-border)] z-40 hidden md:flex flex-col h-screen transition-colors">
      <div className="px-5 py-5 flex items-center space-x-2.5 border-b border-[var(--c-border)]">
        <div className="w-7 h-7 rounded-md bg-[var(--c-accent)] flex items-center justify-center text-white font-bold text-sm">D</div>
        <h1 className="text-base font-semibold text-[var(--c-text)] tracking-tight">DSA Tracker</h1>
      </div>
      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto mt-3 pb-6">
        <p className="px-3 text-[10px] font-semibold text-[var(--c-text-3)] uppercase tracking-widest mb-2">Menu</p>
        {navItems.map((item) => (
